  ...
  methods: {
    ...
    async addFavorite(recipeId) {
      try {
        ...
        this.showPopup('Votre recette a été ajoutée aux favoris avec succès !', 'success');
      } catch (error) {
        console.error('Erreur lors de l\'ajout aux favoris:', error);
        this.showPopup('Cette recette est déjà dans vos favoris !', 'error');
      }
    },
    async sendComment() {
      try {
    ...
    showPopup(message, type) {
      ...
      if (type === 'success') {
        ...
        this.isErrorPopupVisible = false; // Hide the error popup if it is showing
        setTimeout(() => {
          this.isSuccessPopupVisible = false;
        }, 2000); // Fermer automatiquement la fenêtre contextuelle après 2 secondes
      } else if (type === 'error') {
        this.isErrorPopupVisible = true;
        this.isSuccessPopupVisible = false; // Hide the success popup if it is showing
        setTimeout(() => {
          this.isErrorPopupVisible = false;
        }, 2000); // Fermer automatiquement la fenêtre contextuelle après 2 secondes
      }
      console.log("Popup Message:", this.popupMessage);
      console.log("Success Popup Visible:", this.isSuccessPopupVisible);
      console.log("Error Popup Visible:", this.isErrorPopupVisible);
    }
  },
  mounted() {
...
<hr class="block md:hidden mr-20 ml-20 h-1 mt-20 my-4 bg-amber-700 border-0 rounded dark:bg-gray-700">
  ...
  <!-- ******************************************************************************Debut card recette Image*************************************************************************** -->


<!-- Titre -->

<div class="flex justify-center items-center mt-10 mb-10">
    <h1 class="break-words block max-w-sm p-6 ml-2 mr-2 bg-red-950 border border-red-800 rounded-lg text-amber-700 font-serif text-4xl md:text-5xl lg:text-6xl xl:text-7xl whitespace-normal">{{ recipe.title }}</h1>
</div>
...
<div class="flex justify-center">
    <div class="block max-w-lg p-10 ml-2 mr-2 bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <h1 class=" mb-2 text-2xl md:text-4xl text-center font-bold tracking-tight text-red-950 dark:text-white">Description</h1>
    ...
    <div class="max-w-lg mx-auto ml-2 mr-2">
        <a href="#" class="block p-4 bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <div class="flex items-center justify-center mb-4">
                <h1 class="text-xl md:text-4xl font-bold tracking-tight text-red-950 dark:text-white whitespace-normal">Ustensiles nécessaires</h1>
            </div>
            <div class="flex items-center">
                <img src="/image/spatula.png" alt="Ustensiles nécessaires" class="w-6 h-6 mt-1 inline-block">
                <ul class=" flex-wrap list-disc ml-8 mt-2 md:text-2xl text-amber-700">
                    <li v-for="utensil in recipe.utensils" class="break-words">{{ utensil }}</li>
                </ul>
            </div>
        </a>
    </div>
...
<div class="flex justify-center">
    <div class="max-w-xs mx-auto mr-2 ml-2">
        <a href="#" class="block p-4 bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <div class="flex items-center justify-center mb-4">
                <h1 class="text-xl md:text-4xl md:text-center font-bold tracking-tight text-red-950 dark:text-white whitespace-normal">Ingrédients nécessaires</h1>
            </div>
            <div class="flex items-center">
                <img src="/image/diet.png" alt="Ingredients" class="w-6 h-6 mt-1 inline-block">
                <ul class="flex-wrap list-disc ml-8 mt-2 md:text-2xl text-amber-700 ">
                    <li v-for="ingredient in recipe.ingredients" class="break-words">{{ ingredient }}</li>
                </ul>
            </div>
        </a>
    </div>
...
<div class="flex justify-center">
    <div class="flex justify-between">
        <a href="#" class="block max-w-lg p-6 ml-2 mr-2 bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <h1 class="mb-2 text-2xl md:text-5xl font-bold text-center tracking-tight text-red-950 dark:text-white">Recette</h1>
            <div class="list-disc ml-8 mt-2 md:text-xl text-amber-700 ">
...
<div class="block max-w-sm p-6 ml-2 mr-2 bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 md:mx-auto">
    <h1 class="mb-2 text-lg font-bold tracking-tight text-red-950 dark:text-white text-center">Ajustez les proportions en fonction du nombre de personnes.</h1>
</div>
...
<div>
    <div class="block max-w-sm p-6 ml-2 mr-2 mt-5 bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 md:mx-auto" v-if="isUserLoggedIn">
      <button @click="addFavorite(recipe.id)" data-tooltip-target="tooltip-recipes" class="inline-flex flex-col items-center justify-center px-5 dark:hover:bg-gray-800 group">
        <h1 class="mb-2 text-lg font-bold tracking-tight text-red-950 dark:text-white text-center">Ajoute cette recette à tes favoris !</h1>
    ...
    <!-- Popup de succès -->
    <div v-if="isSuccessPopupVisible" class="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50">
      <div class="bg-green-500 text-white p-8 rounded-xl shadow-md">
        <p>{{ popupMessage }}</p>
      </div>
    </div>

    <!-- Popup d'erreur -->
    <div v-if="isErrorPopupVisible" class="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50">
      <div class="bg-red-500 text-white p-8 rounded-xl shadow-md">
        <p>{{ popupMessage }}</p>
      </div>
    </div>
...
<div class="flex justify-center mb-20" v-if="isUserLoggedIn">
...
<div v-for="comment in comments" :key="comment.id" class="block p-4 md:p-8 mt-10 mb-20 lg:mb-0 max-w-xs md:max-w-md lg:max-w-lg mx-auto bg-white border border-red-950 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
