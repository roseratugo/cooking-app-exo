  ...
  methods: {
    ...
    async getFavoriteRecipes() {
      try {
        ...
        const response = await axios.get("http://localhost:3000/favorite/all", {
          headers: {
    ...
    async removeFromFavorites(recipeId) {
      try {
       ...
       const response = await axios({
        method: 'delete',
        url: `http://localhost:3000/favorite/delete/${recipeId}`,
        headers: {
...
<template>
          ...
          <div class="hidden sm:block">
            ...
            <router-link to="/allrecipe">
              <button data-tooltip-target="tooltip-recipes " type="button"
                ...
                <img class="w-6 h-6 mb-1 ml-5 mt-5 text-gray-500 dark:text-gray-400 " src="/image/cook-book.png"
                  alt="Icône de la barre de navigation">
  ...
  <!-- ******************************************************************************************Fin Image************************************************************************* -->
    ...
    <!-- ***********titre recettes************* -->
    <div class="flex justify-center items-center mt-10 rounded-lg border-amber-700 bg-red-950 ml-2 mr-2">
      <span class="text-3xl lg:text-4xl xl:text-5xl font-serif italic p-4 rounded-lg text-amber-700 sm:text-center"> Tes
        recettes favorites </span>
    </div>
    ...
    <div class="flex flex-wrap justify-center">
      ...
      <div class="flex flex-wrap justify-center mb-20">
        ...
        <div v-for="recipe in credentials.recipes" :key="recipe.id"
          class="max-w-sm mt-10 ml-2 mr-2 md:mr-10 flex flex-wrap justify-center bg-white border border-red-950 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 items-center sm:flex-row sm:justify-start">
          <a :href="'/recipe/' + recipe.id" class="w-full"> <!-- Ajout de la classe w-full -->
            <!-- Utilise l'attribut 'image_url' de la recette pour afficher l'image -->
            <img class="rounded-t-lg w-full md:h-100 object-cover" :src="recipe.Recipe.image_url" alt="#" />
          </a>

          <div class="p-5 text-center sm:text-left">
            <a :href="'/recipe/' + recipe.Recipe.id">
              <!-- Affiche le titre de la recette -->
              <h5 class="truncate mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                {{recipe.Recipe.title}}</h5>
            </a>
            <!-- Affiche la description de la recette -->
            <p class="truncate mb-3 font-normal text-gray-700 dark:text-gray-400 text-sm">{{recipe.Recipe.description}}
            </p>
            <!-- Bouton pour voir la recette -->
            <button @click="GoToRecipe(recipe.Recipe.id)"
            ...
            <button @click="removeFromFavorites(recipe.id)"
              class="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 mt-5 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-red-200 via-red-300 to-yellow-200 group-hover:from-red-200 group-hover:via-red-300 group-hover:to-yellow-200 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400">
              <span
                class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                Supprimer
    ...
    <div id="goToTop" class="fixed z-50 bottom-20 right-5">
      <div
        class="card w-16 h-16 bg-white border rounded-full shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
    ...
    <div class="md:hidden"> <!-- Masquer la barre de navigation sur les écrans de taille moyenne et plus grands -->
      <div
        ...
        <div :class="{'grid-cols-4': isUserLoggedIn, 'grid-cols-3 ml-16': !isUserLoggedIn}"
          ...
          <router-link to="/allrecipe">
            <button data-tooltip-target="tooltip-recipes " type="button"
              class=" inline-flex flex-col items-center justify-center px-5  dark:hover:bg-gray-800 group ">
              <img class="w-5 h-5 mb-1 ml-5 mt-5 text-gray-500 dark:text-gray-400 " src="/image/cook-book.png"
                alt="Icône de la barre de navigation">
              <span class="sr-only">Recipes</span>
            </button>
          </router-link>
